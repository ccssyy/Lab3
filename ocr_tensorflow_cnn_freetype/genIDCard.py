...
class gen_id_card(object):
    ...
    def no_noise(self,image,contours,threshold):
        noise_contours = []

        for i in range(len(contours)):
            area = cv2.contourArea(contours[i])
            if area < threshold:
                noise_contours.append(contours[i])
        image_no_noise = cv2.drawContours(image,noise_contours,-1,(255,255,255),-1)
...
if __name__ == '__main__':
    ...
    image_data,label,vec = genObj.gen_captcha_text_and_image()
    #print(vec)
    print(label)
