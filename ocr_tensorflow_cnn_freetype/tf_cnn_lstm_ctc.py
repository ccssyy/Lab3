...
def decode_sparse_tensor(sparse_tensor):
    ...
    for index in decoded_indexes:
        # print("index = ", index)
        result.append(decode_a_seq(index,sparse_tensor))  # print(result)
    return result
...
def get_a_image():
    ...
    image,text,vec = obj.gen_captcha_text_and_image()
    # np.transpose 矩阵转置 (32*256,) => (32,256) => (256,32)
    inputs[0,:] = np.transpose(image.reshape((OUTPUT_SHAPE[0],OUTPUT_SHAPE[1])))/255
...
def get_next_batch(batch_size=128):
    ...
    for i in range(batch_size):
        ...
        image,text,vec = obj.gen_captcha_text_and_image()
        # np.transpose 矩阵转置 (32*256,) => (32,256) => (256,32)
        inputs[i,:] = np.transpose(image.reshape((OUTPUT_SHAPE[0],OUTPUT_SHAPE[1])))/255
        codes.append(list(text))
    targets = [np.asarray(i) for i in codes]
    # print(targets)
    sparse_targets = sparse_tuple_from(targets)
...
def crack_image():
    ...
    learning_rate = tf.train.exponential_decay(INITIAL_LEARNING_RATE,global_step,DECAY_STEPS,LEARNING_RATE_DECAY_FACTOR,
                                               staircase=True)
    ...
    with tf.Session() as session:
        saver.restore(session,"./lstm.model/ocr.model_lstm-6700")
...
def train():
    ...
    def do_report():
        ...
        test_feed = {inputs: test_inputs,targets: test_targets,seq_len: test_seq_len}
        dd,log_probs,accuracy = session.run([decoded[0],log_prob,acc],test_feed)
        return report_accuracy(dd,test_targets)  # decoded_list = decode_sparse_tensor(dd)
    ...
    def do_batch():
        ...
        print("b_cost:",b_cost," steps:",steps)
        if steps > 0 and steps % REPORT_STEPS == 0:
            if (do_report() > 0.9):
                save_path = saver.save(session,"./lstm.model/ocr.model_lstm",global_step=steps)
                print(save_path)
